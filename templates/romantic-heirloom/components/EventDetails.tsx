...
export function EventDetails({ data }: EventDetailsProps) {
  ...
  return (
    <section id="events" className="px-6 py-24 md:px-12 lg:px-24">
      ...
      <div className="mx-auto mt-16 grid max-w-3xl gap-10 md:grid-cols-2">
...
function EventCard({
  ...
  return (
    <div className="rh-card-top-border bg-[var(--tmpl-card)] px-8 py-10 text-center shadow-sm">
      <h3 className="tmpl-display text-2xl font-medium tracking-wide text-[var(--tmpl-fg)]">
        {title}
      </h3>
      <div className="rh-gold-rule mx-auto mt-6 w-16" />
      <div className="tmpl-body mt-8 space-y-6 text-sm font-light">
        <div>
        ...
        <div>
          ...
          <p className="mt-2 font-normal text-[var(--tmpl-fg)]">{venue}</p>
          <p className="mt-1 text-[var(--tmpl-muted)]">{address}</p>
        </div>
      </div>
      {mapsUrl && (
        <div className="rh-photo-border mt-8">
