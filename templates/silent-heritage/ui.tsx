...
export function ShPrimaryButton({
  ...
  return (
    <button
      type="button"
      className={`inline-flex items-center justify-center gap-2 px-8 py-4 text-[10px] font-medium uppercase tracking-[0.3em] transition-opacity hover:opacity-85 disabled:opacity-50 ${styles} ${className}`}
      {...props}
    >
      {children}
    </button>
...
export const shFieldClass =
  "w-full border-0 border-b border-[var(--tmpl-card-border)] bg-transparent px-0 py-3 text-sm text-[var(--tmpl-fg)] outline-none placeholder:text-[var(--tmpl-muted)]/70 focus:border-[var(--tmpl-accent)]";
