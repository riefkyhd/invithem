...
function OrnamentalDivider() {
  return (
    <div className="flex flex-col items-center gap-2 px-2 md:px-4" aria-hidden>
      <span className="text-[8px] text-[var(--tmpl-accent)]">◆</span>
      <div className="h-12 w-px bg-[var(--tmpl-accent)]/50 md:h-16" />
      <span className="text-[8px] text-[var(--tmpl-accent)]">◆</span>
    </div>
  );
}
    ...
    <section className="relative px-6 py-24 md:px-12 lg:px-24">
      <TemplateSectionReveal motion={motion}>
        <div className="text-center">
          ...
          <h2 className="dl-section-title tmpl-display mt-4">{t("countdown")}</h2>
          ...
          {units.map((unit, index) => (
            <div key={unit.label} className="flex items-center">
              <div className="flex flex-col items-center px-3 md:px-6">
                <span className="tmpl-display text-4xl font-light tabular-nums text-[var(--tmpl-accent)] md:text-6xl">
                  {String(unit.value).padStart(2, "0")}
                </span>
                <span className="mt-3 text-[10px] font-light uppercase tracking-[0.3em] text-[var(--tmpl-muted)]">
                  {unit.label}
                </span>
              </div>
