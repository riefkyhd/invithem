    ...
    <section id="wishes" className="px-6 py-24 md:px-12 lg:px-24">
      <TemplateSectionReveal motion={motion}>
        <div className="mx-auto max-w-md text-center">
          ...
          <h2 className="dl-section-title tmpl-display mt-4">{t("wishes")}</h2>
      ...
      <TemplateSectionReveal motion={motion} delay={0.1}>
        <form onSubmit={onSubmit} className="mx-auto mt-12 max-w-md space-y-6">
          ...
          <div>
            ...
            <textarea {...register("message")} rows={4} className="dl-input resize-none" />
            {errors.message && (
      ...
      {wishesLoading ? (
        <div className="mx-auto mt-16 max-w-md">
          <GuestBookSkeleton cardClassName="border-t border-[var(--tmpl-accent)]/30 px-4 py-6" />
        </div>
      ) : (
          ...
          {wishes.length === 0 ? (
            ...
            wishes.map((wish, index) => (
              <TemplateSectionReveal
                ...
                delay={(index % 5) * 0.06}
              >
                <div className="border-t border-[var(--tmpl-accent)]/30 px-4 py-6 text-center">
                  <p className="tmpl-display text-lg font-light tracking-wide">{wish.name}</p>
                  <div className="dl-gold-rule mx-auto mt-3 w-12" />
                  <p className="mt-4 text-sm font-light leading-relaxed text-[var(--tmpl-muted)]">
                    {wish.message}
                  </p>
                </div>
              </TemplateSectionReveal>
