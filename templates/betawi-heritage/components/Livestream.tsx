...
export function Livestream({ data }: LivestreamProps) {
  ...
  const description =
    locale === "id"
      ? "Bagi yang tidak dapat hadir secara langsung, kami mengundang Anda menyaksikan melalui siaran digital kami."
      : t("livestreamDesc");

  return (
    <section id="livestream" className="px-6 py-24 md:px-8">
      <TemplateSectionReveal motion={motion} className="mx-auto max-w-lg">
        <BwEyebrow>{locale === "id" ? "Siaran Langsung" : "Live Stream"}</BwEyebrow>
        <BwSectionTitle className="mt-3">
          {locale === "id" ? "Ikut dari Jauh" : "Join From Afar"}
        </BwSectionTitle>
        <p className="tmpl-body mt-6 text-base leading-relaxed text-[var(--tmpl-muted)]">
          {description}
        </p>
        <div className="relative mt-10 aspect-video overflow-hidden border-2 border-[var(--tmpl-accent)]/30 bg-[var(--tmpl-surface)]">
          <iframe
            src={embedUrl}
